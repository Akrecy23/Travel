...
async function fetchTripsAndRenderTabs(yearId, countryFilter, groupFilter) {
  ...
  itineraryGrid.innerHTML = ""; // Clear old cards
...
async function renderTripsForGroup(currentUserId, yearId, countryId, groupId) {
  ...
  for (const tripDoc of tripsSnap.docs) {
    ...
    card.querySelector(".view-btn").addEventListener("click", () => {
      ...
      console.log(`Open modal for ${yearId} / ${countryId} / ${groupId} / ${tripId}`); // DEBUG PURPOSES
      // Function to show Itinerary Modal (ViewItinerary_Button.js)
      // Pass the trip id (so function know where to nav to in Firestore)
      openItineraryModal(currentUserId, yearId, countryId, groupId, tripId);
    });
  }
}
