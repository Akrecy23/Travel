async function openBookingsModal(currentUserId, yearId, countryId, groupId, tripId) {
  ...
  modal.innerHTML = `
    <div class="modal-box">
      <div class="modal-header">
        <h2 id="modalTitle">${tripId} – Bookings</h2>
  ...
  newBtn.addEventListener("click", () => {
    ...
    openNewBookingForm(tabName, currentUserId, yearId, countryId, groupId, tripId);
  });

  // Initial render
  renderTab("Flights", currentUserId, yearId, countryId, groupId, tripId);
  ...
  modalTabs.querySelectorAll(".tabs-left button").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      renderTab(btn.dataset.tab, currentUserId, yearId, countryId, groupId, tripId);
    });
  });
...
document.addEventListener("BookingsRendered", e => {
  const { tabName, currentUserId, yearId, countryId, groupId, tripId } = e.detail;
  ...
  modalContent.querySelectorAll(".booking-card").forEach(card => {
    ...
    card.querySelector(".delete-btn").addEventListener("click", async () => {
      ...
      if (modalContent.querySelectorAll(".booking-card").length === 0) {
        renderTab(tabName, currentUserId, yearId, countryId, groupId, tripId);
      }
    });
  });
});