...
window.renderTab = async function (expenseId, currentUserId, yearId, countryId, groupId, tripId) {
  ...
  cards.forEach((card) => {
    ...
    deleteBtn.addEventListener("click", async () => {
      ...
      if (modalContent.querySelectorAll(".spending-card").length === 0) {
        renderTab(expenseId, currentUserId, yearId, countryId, groupId, tripId);
      }
    });
  });
  ...
  newBtn.addEventListener("click", () => {
    openNewExpenseForm(expenseId, currentUserId, yearId, countryId, groupId, tripId);
  });
};